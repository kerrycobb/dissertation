...
(sample 006 from \cref{physamples}) which falls outside of the \fowl clade 
inferred by \iqtree.
...
0.1 \cref{fid:dsuite}.
The highest being between \ameri and \wood with a value of 0.098 \cref{fig:dsuite}.
...
lineage \cref{frazao2015,feng2017,portik2023}.
The \cref{frazao2015} phylogeny places \incilius sister to \rhinella rather 
than \incilius which is not supported by most recent studies making 
the approximately 23 mya estimate for the origin of the genus questionable \cref{feng2017,portik2023,pyron2011}. 
\cref{portik2023} estimate the split between \anaxyrus and \incilius to be
20.3 mya (95\% HPD: 17.8-22.5) whereas \cref{feng2017} estimate a much earlier 
age of 16.5 mya (95\% CI: 14.0-19.4).
The dataset from \cref{feng2017} included near complete coverage from 95 nuclear loci 
whereas the \cref{portik2023} has a higher degree of missing data (95\%)
and includes both mitochondrial as well as nuclear loci. 
For these reasons I consider the \cref{feng2017} estimate to be the most reliable
and chose it for the rescaling the branch lengths of the \phycoeval tree.

Scaling the root of the \phycoeval tree I estimated with the \cref{feng2017} estiate
...
\subsection{Historic Introgression}
Despite widespread compatibility among contemporary \anaxyrus species
and the existence of at least three contemporary hybrid zones, there is not 
strong evidence of widespread or extensive introgression among species in the 
evolutionary history of \anaxyrus. 
The highest \fbranch statistics were calculated between \amer and \hemiophrys and 
between \amer and \baxter with values of 0.24 and 0.22 respectively \cref{fig:dsuite}.
One of the contemporary \anaxyrus hybrid zones is one that exists between \amer and \hemiophrys.
It may be that the high \fbranch estimate is a result of this contemporary 
hybridization rather than from past introgression between \amer and \hemiophrys.
The single \hemiophrys sample included in this study is from a location in 
close proximity to the range of \amer \cref{fig:americanus-map} \parentcite{conant1998}.
\cite{green1983} reported clinal variation of allozyme alleles at 5 different loci
across an approximately 100 km transect in southeastern Manitoba, Canada.
However, there is also a high \fbranch score between \amer and \baxteri which do 
not have ranges that are close to one another.
It is possible the introgression took place between \amer and the ancestor 
of \hemiophrys and \baxteri. 
\baxteri is believed to be a relict of a one more southerly distribution of 
\hemiophrys during a Pleistocene glacial period \parencite{henrich1968}.
Unfortunately, it was not possible to directly test for introgression between
\amer and the common ancestor to \hemiophrys and \baxteri due to limitations of 
the \fbranch test \parencite{malinsky2021}.

Several other \fbranch tests returned non-zero values.
Five of the \wood \fbranch statistics were 
More than half of the \wood \fbranch statistics were greater than zero but 
were relatively low compared to the results for \amer \cref{fig:dsuite}.
Hybridization between all of these species is plausible as \wood occurs in 
sympatry at some part of its range with all of them. 
Contemporary hybridization involving \wood has been reported with \amer, 
\microscaphus, and \speciousus. 

% Particularly with \amer, \baxteri, \hemiophrys, and \microscaphus with which it 
% is closely related and with which I found the highest \fbranch scores \cref{fig:phycoeval}.
% Perhaps this is also driven by introgression among common ancestors, 
% which again, I was not able to test directly due to limitation of the 
% \fbranch statistic.
% However, the highest \fbranch value for \wood was for \amer () which could 
% reflect recent or ongoing hybridization between the two as they exist in 
% sympatry at the edges of their respective ranges \cite{conant1998}.
...
subspecies \textit{A. anaxyrus charlesmithi}
...
implications for understanding the contemporary hybridization in \an
