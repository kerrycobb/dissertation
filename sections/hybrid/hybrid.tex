...
It is a gradual and continuous process during which the exchange of genetic  
variants through hybridization can occur \parencite{mallet2008,wu2001}.
...
Aside from having important evolutionary consequences which warrant investigation, 
...
This heterogeneity can arise via genetic drift occurring within hybrid populations,
...
Much of this heterogeneity is the result of divergent selection acting in different 
ways in each species \parencite{nosil2009}.
Regions with particularly high levels of divergence between species have been 
...
proportions, cline models, and measurement of parental population differentiation.
...
ancestry, $\alpha$ outliers with greater expected \terr ancestyr, and estimates 
...
\section{Discussion}
In this study, I investigated the patterns of genetic variation across putative
a hybrid zone between two species of toads using genome wide sequence data 
collected from samples distributed across the hybrid zone. 
...
of \amer and \terr seems to bear the hallmarks of a "tension zone" \parencite{barton1985}.
A tension zones is a hybrid zone model which is characterized by abrupt genotypic 
clines maintained by a balance between dispersal and selection against certain
hybrid genotypes \parencite{barton1985}.
Tension zones are expected to correspond with natural features that reduce    
dispersal or density of the parent species.
The ranges of \amer and \terr abut with an abrupt transition and no apparent
overlap. 
...
Such a sudden transition is difficult to explain except that it is the result of 
...
The admixture coefficients inferred in the \structure analysis span the entire 
range from 0.05\%-0.5\% which would be expected if hybrids are viable, fertile,
and capable of backcrossing over multiple generations \citationNeeded \cref{fig:hybrid-main}. 
...
Inferred admixture coefficients decrease and approach zero with distance
from the center of the hybrid zone \cref{fig:hybrid-main}.
The results of the \structure analysis indicate that the width of the hybrid
zone is quite wide. 
At least as wide as the sampling area as there are samples with appreciable
admixture proportions near the Northeastern and Southwestern edges of 
the sampling area \cref{fig:hybrid-main}.

The width of a hybrid zone is largely determined by selection and dispersal.

The reason for a low level of admixture observed even in very distance 
samples could be due to adaptive introgression.


% Presence of non-admixed individuals near the center of the hybrid zone
% suggests that toads have a high dispersal. I think I should leave this out. 
% This would also be one thing that could explain the large width of the hybrid zone.
% It is also possible that the hybrid zone is not even but mosaic. 
% The number and distribution of samples aren't adequate to say if this is the case


It is also possible that some of this inferred admixture is the result of a 
statistical artifact or due to error. 
Some reassurance is provided by the result of the PCA which is consistent with 
the \structure results although it is possible that they could be affected 
by the same bias or error introduced at by data collection and processing \cref{fig:hybrid-main} \citationNeeded.


% The most intermediate samples are found near the center with increasing purity
% with distance from the contact zone with a couple of outliers.
% There are a lot of pure samples, even close to the center of the hybrid zone
% which suggest selection against introgression must be high.

% These outliers could be the result of artifacts in the analysis or errors made
% in the lab.



\subsection{Variability of introgression}
Used genomic cline instead of geographic cline.
Theory and evidence point to a balance between migration and selection as the 
cause for the creation and persistence of hybrid zones .
Organisms migrate and move into one another's ranges.

Hybrid zones are thought to persist because of a balance between selection
and migration.
